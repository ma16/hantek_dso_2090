...
/* [ma16] [info] actually there are a lot more possible timebases since 
   there is a prescaler that supports 3 + 2^16 different dividers */
#define NUM_VDIV     8

#define NUM_BUFFER_SIZES 2
/* [ma16] [info] there are three different frame sizes */
...
/* [ma16] [info] i don't see any way to query configuration values. 
   sigrok appears to return only its own presets */

/* [ma16] [defect] sigrok uses always its own presets + command-line 
   arguments for device configuration. Command-line arguments permit 
   either to set devopts or devopts_cg. But not both. So they become
   mutual exclusive! For instance, it is not possible for a user
   to set TIMEBASE and VDIV. */
...
/* [ma16] [info] there is also a frame for 256 records */
...
/* [ma16] [question] OpenHantek deals with different devices in different 
   ways. Has this been really considered in this implementation here? */
...
/* [ma16] [note] I would consider getting a frame unconditionally as 
   quite important (marked as TODO above). */
...
/* [ma16] [question] I can't see an option for GND, do you? */
	...
	for (int ch = 0; ch < 2; ch++) {
		...
		for (int i = 0; i < num_samples; i++) {
			 ...
			 */
		  	...
		  	/* [ma16] [info] if only one channel is enabled, both ADCs
			   sample the same signal with an offset of 1/(2*f) which
			   enables oversampling (e.g. 100 MHz instead of 50 MHz) */
