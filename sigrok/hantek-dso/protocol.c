...
static int dso_set_trigger_samplerate(const struct sr_dev_inst *sdi)
{
	...
	/* [ma16] [defect] note that sample_rate := 25e+6 / (~divider + 2) 
	   that would be: 1.25e+7, 5.0e+6, 2.5e+6, 
	                   1.0e+6, 490196, 247524, 
			   1.0e+5, 5.0e+4, 2.5e+4, 
			   1.0e+4, 5.0e+4, 2.5e+3 */
	uint16_t timebase_large[] = { 0xffff, 0x0000, 0xfffc, 0xfff7, 0xffe8,
		0xffce, 0xff9d, 0xff07, 0xfe0d, 0xfc19, 0xf63d, 0xec79 };
	/* [ma16] [defect] I don't see why prescaler values should 
	   depend on the frame-size. Unless you would like to match the
	   frame-size according to TIMEBASE as ratio: 10k/32k (which 
	   above code does not).
	...
	if (devc->framesize == FRAMESIZE_SMALL) {
	        /* [ma16] [question] why make it depend on the frame-
		   size in this particular way? */
		if (devc->timebase < TIME_20us)
		  	tmp = 0; /* [ma16] [info] 50 MHz */
		else if (devc->timebase == TIME_20us)
			tmp = 1; /* [ma16] [info] 50 MHz */
		else if (devc->timebase == TIME_40us)
			tmp = 2; /* [ma16] [info] 25 MHz */
		else if (devc->timebase == TIME_100us)
			tmp = 3; /* [ma16] [info] 10 MHz */
		else if (devc->timebase >= TIME_200us)
			tmp = 4; /* [ma16] [info] use divider (below) */
	} else {
		if (devc->timebase < TIME_40us) {
			sr_err("Timebase < 40us only supported with 10K buffer.");
			/* [ma16] [question] where frome comes this? */
			return SR_ERR_ARG;
		}
		else if (devc->timebase == TIME_40us)
			tmp = 0; /* [ma16] [info] 50 MHz */
	...
	/* Fast rates channel */
	/* TODO: Is this right? */
	/* [ma16] [info] I can't see any effect when setting this bit */
	...
	if (devc->timebase < TIME_100us)
	  	tmp = 0; /* [ma16] [question] 381 samples per second!? */
	else if (devc->timebase > TIME_400ms)
	  	tmp = 0xffed; /* [ma16] [info] 1.25e+6 */
	else {
		if (devc->framesize == FRAMESIZE_SMALL)
			tmp = timebase_small[devc->timebase - 3];
		else
			tmp = timebase_large[devc->timebase - 3];
		/* [ma16] [question] are you sure about this? */
	}
	...
	/* [ma16] [info] The counter (aka trigger-position) starts 
	   counting up to 0x80000 after the trigger has been released. 
	   Another sample is recorded with each step. 
	   [defect] So this implementation might be right with a 32k 
	   frame-size but not with a 10k frame-size. It should also be 
	   noted that pre-trigger samples may be anything (i.e. 
	   garbage) unless it has been made sure there was sampling in
	   progress. */
...
static int dso_set_voffsets(const struct sr_dev_inst *sdi)
{
	...
	/* [ma16] [question] why "| 0x20" ? (seems to have no effect) */
...
{
  	/* [ma16] [info] this function appears to be never called 
	   however, this is the only way to aquire a frame without
	   knowledge of the signal form (i.e. unconditionally). */
	struct sr_usb_dev_inst *usb;
	int ret, tmp;
	uint8_t cmdstring[2];
...
SR_PRIV int dso_init(const struct sr_dev_inst *sdi)
{
	...
	if (dso_enable_trigger(sdi) != SR_OK)
		return SR_ERR;
	/* [ma16] [defect] this uses the preset (default) configuration
	   values + either devopts _or_ devopts_cg configuration (but
	   not both) (already mentioned before) */
		...
		uint8_t *capturestate, uint32_t *trigger_offset)
{
	...
	*capturestate = inbuf[0];
	...
	/* [ma16] [defect] inbuf[3] belongs to the next offset */
